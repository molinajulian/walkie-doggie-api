...
describe('POST /sessions/refresh', () => {
  ...
  describe('Fail because the access token is expired', () => {
    it('Should return status code 401', () => {
      expect(expiredTokenResponse.statusCode).toEqual(401);
    });
    it('Should return internal_code invalid_token', () => {
      expect(expiredTokenResponse.body.internal_code).toBe('invalid_token');
    });
    it('Should return a message indicating the provided token is expired', () => {
      expect(expiredTokenResponse.body.message).toEqual('jwt expired');
    });
  });
});
